...
%\red{[[Mention A+ being funded? How will this improve each parameter?]]}
...
%\red{[[Must mention evaluation time to segue into next 2 paragraphs.]]}
...
%The resulting surrogate is the first EOB waveform model that consistently incorporates spin and tides and is fast enough for production parameter estimation codes.
...
\subsection{Quadrupole-monopole terms}


When we have a binary with two material bodies, like two neutron stars, 1 and 2, which have spin-induced mass quadrupole moments $Q_{1,\mr{SS}}^{ij}=-\kappa_1 S_1^{\langle i}S_1^{j\rangle}/m_1$, similarly with $1\to 2$, as opposed to the case with two black holes where $\kappa_1=\kappa_2=1$, the SEOB model should be modified as follows in order to take this into account at leading order.

The SEOB effective Hamiltonian is given by (5.70) of \cite{Barausse:2009xi} as
\be
H_\mr{eff}=H_S+\beta^ip_i+\alpha\sqrt{\mu^2+\gamma^{ij}p_ip_j+\mc Q_4(p)}+H^\mr{BBH}_\mr{extra},
\ee
where
\be
H^\mr{BBH}_\mr{extra}=\frac{1}{2r^3}(3n_in_j-\delta_{ij})\frac{\mu}{M}S_*^iS_*^j,\qquad\qquad S_*^i=\frac{m_2}{m_1} S_1^i+\frac{m_1}{m_2}S_2^i.
\ee
This should be replaced with
\begin{alignat}{3}
H^\mr{BNS}_\mr{extra}&=\frac{1}{2r^3}(3n_in_j-\delta_{ij})\bigg[\frac{\mu}{M}S_*^iS_*^j+(\kappa_1-1)\frac{m_2}{m_1}S_1^i S_1^j+(\kappa_2-1)\frac{m_1}{m_2}S_2^i S_2^j\bigg] \label{eq:HBNSextra}
\\\nnm
...
\end{alignat}
This could also be expressed solely in terms of $\bs S_*$ and $\bs S=\bs S_1+\bs S_2$, instead of $\bs S_1$ and $\bs S_2$, but then the mass difference $m_1-m_2$ would appear in denominators, which might cause problems with dividing by zero in the equal mass case.

In the waveform, since we are focusing only on the leading 2PN order, the only contribution to consider will be in the mode $h_{22}$. The \emph{total} leading-order spin-squared contribution, for aligned-spin circular orbits, is given by
\be
h_{22}^\tr{LO-SS}=-\frac{8\nu M^3\omega^2}{R}\sqrt{\frac{\pi}{5}}e^{-2i\Phi}\mc A,
\ee
and the corresponding contribution to the flux is
\be
\mc F^\tr{LO-SS}=\frac{32\nu^2}{5}x^7\left[2\mc A+\frac{1}{16M^2}\left(\frac{S_1}{m_1}-\frac{S_2}{m_2}\right)^2\,\right],\qquad\qquad x=(M\omega)^{2/3},
\ee
where, for a BBH,
\be\label{eq:ABBH}
\mc A^\mr{BBH}=\frac{1}{M^2}\left(\frac{S_1^2}{m_1^2}+2\frac{S_1S_2}{m_1m_2}+\frac{S_2^2}{m_2^2}\right).
\ee
For a BNS, this should be replaced with
\be\label{eq:ABNS}
\mc A^\mr{BNS}=\frac{1}{M^2}\left(\kappa_1\frac{S_1^2}{m_1^2}+2\frac{S_1S_2}{m_1m_2}+\kappa_2\frac{S_2^2}{m_2^2}\right).
\ee
The $h_{22}$ then matches (4.26), (4.27b), (3.4) of \cite{Buonanno:2012rv}, and the flux matches e.g.\ (6.18)--(6.19) of \cite{Marsat:2014xea}.

In the EOB factorized waveform (see e.g. (17) in~\cite{Taracchini:2012ig}), spin contributions in the effective source, tail factors and phase contribution would enter only at higher order, so that the only modification is in the $\rho_{22}$ factor. We have simply
\be\label{eq:rho22}
	\rho_{22}^{\textrm{LO-SS}} = \frac{1}{2}\mathcal{A} v^{4} \,,
\ee
with $\mathcal{A}$ the quantity given by~\eqref{eq:ABBH} or~\eqref{eq:ABNS} and $v=x^{1/2}$.

%Though this should be ultimately irrelevant for our purposes, just for completeness, recall the following:  The LO-SS terms in the EOB effective Hamiltonian, specifically the $S_\mr{Kerr}^2$ terms, are not in the usual PN ``gauge.''  This is seen e.g.\ in (5.55) and (5.60) of \cite{Barausse:2009xi}; the last three lines of (5.60) are equivalent  to zero via a  canonical transformation [the one in (5.15), (5.20)].  If all we care about is getting the SS terms in the Hamiltonian correct at LO, regardless of gauge, then the above prescriptions are sufficient, even though some SS terms are in a different gauge than others.  (The BBH SEOB Hamiltonian already has this feature anyway.)

In the Hamiltonian, we separate in Eq.~\eqref{eq:HBNSextra} the additional contribution for BNS, as
\be
	\Delta H_{\rm SS}^{\rm BNS} = H^{\rm BNS}_{\rm extra} - H^{\rm BBH}_{\rm extra} = \frac{1}{2r^3}(3n_in_j-\delta_{ij})\bigg[(\kappa_1-1)\frac{m_2}{m_1}S_1^i S_1^j+(\kappa_2-1)\frac{m_1}{m_2}S_2^i S_2^j\bigg] \,.
\ee
This contribution is computed by the function \texttt{XLALSimIMRTEOBdeltaHssQuadMonLO}, called inside \texttt{XLALSimIMRSpinEOBHamiltonian} in \texttt{LALSimIMRSpinEOBHamiltonian.c}. The expression implemented is kept generic although the $S\cdot n$ terms are zero for aligned spins.

In the factorized waveform, after subtracting the quadratic-in-spin contribution for BBH, the additional contribution for BNS in~\eqref{eq:rho22} is given by:
\be
  \Delta \rho_{22}^{\rm BNS} = \rho_{22}^{\rm SS} - (\rho_{22}^{\rm SS})_{\kappa_{1,2}=1} = v^{4}\left[ \left(\chi_s^2+\chi_a^2\right) \left(\frac{\delta  \kappa_a}{2} -\kappa_s \nu+\frac{\kappa_s}{2} +\nu-\frac{1}{2}\right) + \chi_a \chi_s \left(-2 \nu  \kappa_a+\kappa_a-\delta +\delta  \kappa_s\right) \right]\,,
\ee
where we used the variables $\kappa_{s,a} \equiv (\kappa_{1} \pm \kappa_{2})/2$, $\chi_{s,a} \equiv (\chi_{1} \pm \chi_{2})/2$. One can readily check that this is zero in the binary black hole case, for $\kappa_{s}=1$ and $\kappa_{a}=0$. This contribution is added, if at least one of the $\kappa_{1,2}$ is different from one, in the function \texttt{XLALSimIMREOBCalcSpinFacWaveformCoefficients} of \texttt{LALSimIMRSpinEOBFactorizedWaveform.c}

To compute the quadrupole-monopole parameters $\kappa_{1,2}$ entering these contributions (represented by their deviation from 1 as \texttt{dQuadMon1}, \texttt{dQuadMon2}), we use a fit taken from~\cite{Yagi:2016bkt} (see Eq.~(15), with coefficients given in the third row of Table~I), with an extension to ensure the black-hole limit with a $C^{2}$-continuity at the matching point at a dimensionless quadrupolar tidal deformability $\overline{\lambda}_{2}=1$ (chosen arbitrarily). This extended universal relation reads
\be
	\kappa(\overline{\lambda}_{2}) = 
	  \begin{cases}
	    \exp \left[\sum_{i=0}^{4} a_{i}\xi^{i} \right] & \text{ if } \overline{\lambda}_{2} \geq 1 \,,\\
	    1 + 0.428\overline{\lambda}_{2} - 0.324\overline{\lambda}_{2}^{2} + 0.111\overline{\lambda}_{2}^{3} & \text{ if } 0 \leq \overline{\lambda}_{2} < 1 \,,
	  \end{cases}
\ee
with $\{a_{i}\}_{i=0,\dots, 4} = \{0.1940, 0.09163, 0.04812, -4.283.10^{-3}, 1.245.10^{-4}\}$, $\xi = \ln \overline{\lambda}_{2}$, and where we rounded off the numerical coefficients of the quadratic extension. This universal relation is implemented in \texttt{XLALSimUniversalRelationQuadMonVSlambda2Tidal} in \texttt{LALSimUniversalRelations.c}. 
...
%This is especially true for strongly correlated parameters such as the pair $q$ and $\chi_{\rm eff}$. 
...
%For data with zero noise, the fit between the injected waveform and the template is best when the likelihood is maximum. However, as a result of the priors that are bounded and not flat for some parameters, the posterior, and especially the marginalized posteriors, will not be maximized at the injected parameters even with a perfect template. We therefore use the difference between the parameters that maximize the likelihood $\vec\vartheta_{\rm max}$ and the injected parameters $\vec\vartheta_{\rm inj}$ as our estimate for the systematic error due to the surrogate. 
%
%Marginalized 1-dimensional posteriors for four key parameters are shown in Fig.~\ref{fig:pe}. These are the chirp mass $\mathcal{M} = (m_1 m_1)^{3/5}/(m_1+m_2)^{1/5}$, the mass ratio $q=m_2/m_1$, the effective spin $\chi_{\rm eff} = (m_1\chi_1+m_2\chi_2)/(m_1+m_2)$ and the effective tidal deformability parameter $\tilde\Lambda = (16/13) [(1+12q)\Lambda_1 + (12+q)q^4\Lambda_2]/(1+q)^5$. The maximum value of the likelihood found for each run is dashed, and the injected value is solid. Because the sampled parameters do not necessarily contain the parameters for the maximum likelihood, the difference between these parameters $\Delta\vec\vartheta_{\rm sys} = \vec\vartheta_{\rm max} - \vec\vartheta_{\rm inj}$ is an upper bound on the systematic error that results from using the surrogate as the template instead of the original \texttt{SEOBNRv4T} waveform. We can compare this systematic error to the statistical error $\Delta\vec\vartheta_{\rm stat}$ from the finite sensitivity of the detectors. Here, we will $\Delta\vec\vartheta_{\rm stat}$ as half the width of the 90\% credible interval. Table~\ref{tab:syserrors} shows that the systematic error is small compared to the statistical errors even for the injection with SNR=60.
...
%\begin{table}[htb]
%\centering    
%\caption{Estimated systematic errors due to surrogate model and statistical errors from finite detector sensitivity. The ratio $\Delta\vec\vartheta_{\rm sys} /\Delta\vec\vartheta_{\rm stat}$ is given for the two injections with network SNRs of 30 and 60.} 
%\begin{tabular}{ccc}        
%\hline
%$\vartheta$ & SNR=30 & SNR=60 \\
%\hline
%$\mathcal{M}$ &  $<$ & $<$ \\
%$q$ & $<$  & $<$ \\
%$\chi_{\rm eff}$ & $<$  & $<$ \\
