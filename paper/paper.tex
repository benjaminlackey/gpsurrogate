...
\begin{equation}
\Lambda_{\ell,\textrm{dyn}}^{A,B}(u)=\frac{2}{(2\ell-1)!!}\frac{k^{A,B}_{\ell}}{C_{A,B}^{2\ell+1}}\hat{k}_{\ell\,\textrm{dyn}}(u;\omega_{0\ell}^{A,B})\,,
\end{equation}
where $k^{A,B}_{\ell}$ is the tidal Love number and $C_{A,B}=R_{A,B}/M_{A,B}$ is the neutron star compactness, with $R_{A,B}$ the neutron star radius. Here $\hat{k}_{\ell\,\textrm{dyn}}(u;\omega_{0\ell}^{A,B})$ is the separation-dependent, dimensionless enhancement factor given in Eq.~(11) of Ref.~\cite{Dietrich:2017feu}, which depends on the value of the $f$-mode $2^\ell$-pole angular frequency, $\omega_{0\ell}^{A,B}$. 
