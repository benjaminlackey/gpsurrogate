...
As a final test of the surrogate, we perform parameter estimation runs where we inject the original, time-domain \texttt{SEOBNRv4T} waveform and recover the injected parameters with the \texttt{SEOBNRv4T\_surrogate} template. For the two aLIGO detectors we use the design sensitivity PSD~\cite{}, and for the advanced Virgo detectors we also use its design sensitivity PSD. We choose as our waveform parameters $m_1=m_2=1.4M_\odot$, $\chi_1=\chi_2=0.1$, and $\Lambda_1=\Lambda_2=1286$, corresponding to the MPA1 equation of state. The sky location and inclination angle are fixed at an arbitrary value, and the distance is chosen such that the optimal network signal-to-noise ratio (SNR) is either 30 or 60. We inject the waveforms into zero noise data so that the recovered parameters do not depend on the specific noise realization~\cite{}.

As discussed in more detail in~\cite{BNSPE}, the posterior $p(\theta | d)$ for the parameters $\theta$ is given by Bayes' theorem:
\begin{equation}
p(\theta | d) \propto p(\theta) \mathcal{L}(d | \theta),
\end{equation}
where $p(\theta)$ is the prior and $\mathcal{L}(d | \theta)$ is the likelihood. We choose for our priors the sky position and orientation uniform on the unit sphere. The distance is uniform in co-moving volume. The masses, spins, and tidal parameters are uniform in the ranges $m_{1, 2} \in [0.7, 2]M_\odot$, $\chi_{1,2} \in [-0.5, 0.5]$ and $\Lambda_{1,2} \in [0, 5000]$. We sample this posterior with the parallel-tempered MCMC code in LALInference. We sample the data and waveform at \red{8192}~Hz such that the Nyquist frequency is 4096~Hz. \red{[[We note that published analyses for BNS systems typically use a sampling frequency of 4096~Hz. However, the \texttt{SEOBNRv4T} waveform model still has some signal above the corresponding Nyquist frequency of 2048~Hz (See Fig.~\ref{fig:hoff}).]]} We integrate the likelihood function in the interval $[20, 4096]$~Hz. Approximately \red{$10^x$} iterations are needed to produce \red{$\sim 5000$} statistically independent samples.

For data with zero noise, the fit between the injected waveform and the template is best when the likelihood is maximum. However, as a result of the priors that are bounded and not flat for some parameters, the posterior, and especially the marginalized posteriors, will not be maximized at the injected parameters even with a perfect template. We therefore use the difference between the parameters that maximize the likelihood $\theta_{\rm max}$ and the injected parameters $\theta_{\rm inj}$ as our estimate for the systematic error due to the surrogate. 

Marginalized 1-dimensional posteriors for four key parameters are shown in Fig.~\ref{fig:pe}. These are the chirp mass $\mathcal{M} = (m_1 m_1)^{3/5}/(m_1+m_2)^{1/5}$, the mass ratio $q=m_2/m_1$, the effective spin $\chi_{\rm eff} = (m_1\chi_1+m_2\chi_2)/(m_1+m_2)$ and the effective tidal deformability parameter $\tilde\Lambda = (16/13) [(1+12q)\Lambda_1 + (12+q)q^4\Lambda_2]/(1+q)^5$. The maximum value of the likelihood found for each run is dashed, and the injected value is solid. Because the sampled parameters do not necessarily contain the parameters for the maximum likelihood, the difference between these parameters $\Delta\theta_{\rm sys} = \theta_{\rm max} - \theta_{\rm inj}$ is an upper bound on the systematic error that results from using the surrogate as the template instead of the original \texttt{SEOBNRv4T} waveform. We can compare this systematic error to the statistical error from the finite sensitivity of the detectors. We will define this as half the distance between the 5\% and 95\% percentiles of each parameter: $\Delta\theta_{\rm stat} = (\theta_{95} - \theta_{5})/2$. Table~\ref{tab:syserrors} shows that the systematic error is small compared to the statistical errors even for the injection with SNR=60.
...
\caption{Marginalized PDFs for two waveform injections with zero noise. The parameters are identical except for the distances which are set such that the network SNRs are 30 (blue) or 60 (red). The shaded regions are bounded by the 5 and 95 percentiles. The vertical solid black line represents the injected parameter value. The vertical dashed lines represent the parameter value with maximum likelihood.}
\label{fig:pe}
\end{figure}

\begin{table}[htb]
\centering    
\caption{Estimated systematic errors due to surrogate model and statistical errors from finite detector sensitivity. The ratio $\Delta\theta_{\rm sys} / \Delta\theta_{\rm stat}$ is given for the two injections with network SNRs of 30 and 60.} 
\begin{tabular}{ccc}        
\hline
$\theta$ & SNR=30 & SNR=60 \\
\hline
...
BL thanks the participants of the Bayesian Methods in Nuclear Physics workshop at the Institute for Nuclear Theory where many of the methods used here were discussed. The authors were supported by... 
