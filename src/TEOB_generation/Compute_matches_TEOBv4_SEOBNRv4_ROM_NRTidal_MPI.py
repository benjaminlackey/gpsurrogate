...
def BNS_tidal_match(i, m1, m2, chi1, chi2, lambda1, lambda2, tmpdir,
                    f_min=40.0, f_max=8192.0, deltaF=1/128.0, allow_skip=True):
    ...
    try:
        fTEOB, HpTEOB, HcTEOB = spin_tidal_eob_FD(m1, m2, chi1, chi2, lambda1, lambda2,
                        f_min, f_max, deltaF=deltaF,
                        distance=distance, inclination=inclination,
                        approximant='TEOBv4')
        
        fNRT, HpNRT, HcNRT = spin_tidal_eob_FD(m1, m2, chi1, chi2, lambda1, lambda2,
                        f_min, f_max, deltaF=deltaF,
                        distance=distance, inclination=inclination,
        ...
        m = match(HpTEOB.data.data[::s], HpNRT.data.data[::s], 
              LS.SimNoisePSDaLIGOZeroDetHighPower, 
              deltaF*s, zpf=2, verbose=False)

        np.save(tmpdir+'match_'+str(i)+'.npy', np.array([m1, m2, chi1, chi2, lambda1, lambda2, m]))
        np.save(tmpdir+'wfs_'+str(i)+'.npy', np.array([fTEOB, HpTEOB.data.data, HpNRT.data.data]))
