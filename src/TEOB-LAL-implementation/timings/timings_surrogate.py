...
# Use same \Delta f values as used for SEOBNRv4T
dfs_SEOBNRv4T = np.array([0.000488, 0.001953, 0.003906, 0.007812, 0.015625, 0.015625, 0.03125 , 0.03125 , 0.03125 , 0.0625  ,
       0.0625  , 0.0625  , 0.125   , 0.125   , 0.125   , 0.125   , 0.25    , 0.25    ])

tim_sur = np.array([timeit_ipython("FD_waveform_test(Mtot, x, LS.SEOBNRv4T_surrogate, fLow=%f, fHigh=2048.0, deltaF=%f)" %(f_min, df), number=number) 
 for f_min, df in zip(f_mins, dfs_SEOBNRv4T)])
np.save('tim_sur_additional', tim_sur)
